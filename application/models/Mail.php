...
class Mail extends School 
{
    ...
    //Send contact form.
    function sendForm(){
        $email_sub   = getEduAppGTLang('new_contact_message');
        $email_msg   = '<b>'.getEduAppGTLang('message_details').'</b><br><br>';
        $email_msg   = getEduAppGTLang('name').":<b> ".$this->input->post('name').'</b><br>';
        $email_msg   = getEduAppGTLang('phone').":<b> ".$this->input->post('phone').'</b><br>';
        $email_msg   = getEduAppGTLang('email').":<b> ".$this->input->post('email').'</b><br>';
        $email_msg   = getEduAppGTLang('message').":<b> ".$this->input->post('message');
        $email_to    = $this->db->get_where('settings', array('type' => 'system_email'))->row()->description;
        $email_data = array(
            'email_msg' => html_entity_decode($email_msg)
        );
        if($email_to != ''){
